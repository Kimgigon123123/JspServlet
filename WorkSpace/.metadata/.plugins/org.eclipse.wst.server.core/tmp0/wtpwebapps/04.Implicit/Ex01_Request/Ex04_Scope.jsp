...
<body>
	...
	<p>Page Context (JSP 페이지 내부에서만 데이터를 전달하는 목적으로 사용되는 객체. (사용빈도가 매우 낮음))</p>
	...
	<p>ApplicationContext ( 어플리케이션이 완전히 종료되지 않는 한 계속 해서 유지됨 -> 버전 정보, 특정한 업데이트를 위한 값 외에는 저장x)</p>
	
	<%
		pageContext.setAttribute("pageCtx", "KGG_PAGE_CTX"); //페이지 내에서만 사용하는 객체
		request.setAttribute("req", "KGG_REQ");
		// Request = > 1. getPrameter (URL 또는 FROM 태그를 이용한 경우 내부 태그를 name으로 구분하여 파라메터가 String으로 들어있음)
		//    	   = > 2. Attribute < = 어떤 요청을 통해서 오거나 Request가 Froward 라는 기능으로 페이지 전환을 하는경우 Object가 들어있음
		session.setAttribute("session","KGG_SESSION");
		application.setAttribute("app", "KGG_APPV1.1");
